...
do fc = 1,num_filter
  ...
  !-----------------------------------------------------------------------
  !#py subroutine Prepare_Data_for_EM_PTM_Driver(bounds, num_filter, filter, &
!#py !#py        sabg_lyr, dhsdT, hs_soil, hs_top_snow, hs_h2osfc)
!#py     !
!#py     ! !DESCRIPTION:
!#py     ! Prepare data needed for the external model, PETSc-based Thermal
!#py     ! Model (PTM).
!#py     !
!#py     ! !USES:
...
!#py     integer                , intent(in)    :: num_filter                                         ! number of columns in the filter
!#py     integer                , intent(in)    :: filter(:)                                          ! column filter
!#py     real(r8)               , intent(in)    :: sabg_lyr(bounds%begc:bounds%endc,-nlevsno+1:1)     ! absorbed solar radiation (col,lyr) [W/m2]
!#py     real(r8)               , intent(in)    :: dhsdT(bounds%begc:bounds%endc)                     ! temperature derivative of "hs" [col]
!#py     real(r8)               , intent(in)    :: hs_soil(bounds%begc:bounds%endc)                   ! heat flux on soil [W/m2]
!#py     real(r8)               , intent(in)    :: hs_top_snow(bounds%begc:bounds%endc)               ! heat flux on top snow layer [W/m2]
!#py     real(r8)               , intent(in)    :: hs_h2osfc(bounds%begc:bounds%endc)                 ! heat flux on standing water [W/m2]
!#py     !
!#py     ! !LOCAL VARIABLES:
!#py     integer                                :: c, j
!#py 
!#py     associate(                                                      &
!#py          eflx_sabg_lyr    => col_ef%eflx_sabg_lyr,     &
!#py          eflx_dhsdT       => col_ef%eflx_dhsdT ,       &
!#py          eflx_hs_soil     => col_ef%eflx_hs_soil ,     &
!#py          eflx_hs_top_snow => col_ef%eflx_hs_top_snow , &
!#py          eflx_hs_h2osfc   => col_ef%eflx_hs_h2osfc     &
!#py          )
!#py 
!#py       do c = bounds%begc, bounds%endc
!#py          do j = -nlevsno+1, 1
!#py             eflx_sabg_lyr(c,j)  = sabg_lyr(c,j)
!#py          enddo
!#py 
!#py          eflx_dhsdT(c)       = dhsdT(c)
!#py          eflx_hs_soil(c)     = hs_soil(c)
!#py          eflx_hs_top_snow(c) = hs_top_snow(c)
!#py          eflx_hs_h2osfc(c)   = hs_h2osfc(c)
!#py       enddo
!#py 
!#py     end associate
!#py 
!#py   end subroutine Prepare_Data_for_EM_PTM_Driver
