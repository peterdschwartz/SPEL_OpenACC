...
contains
  ...
  subroutine SoilLittVertTransp(num_soilc, filter_soilc,   &
    ...
    real(r8) :: bet
    real(r8) :: gam(num_soilc,0:nlevdecomp+1,ndecomp_pools)
    real :: startt, stopt
    !-----------------------------------------------------------------------
    ...
    associate(                                                      &
      ...
      do i_type = 1, ntype
         ...
         if (use_vertsoilc) then
            ...
            !$acc parallel loop independent gang worker vector collapse(2) default(present) private(bet)
            do s = 1, ndecomp_pools
               do fc = 1,num_soilc
                  if(.not. is_cwd(s)) then
                     bet = b_tri(fc,0,s)
                     ...
                     do j = 0, nlevdecomp+1
                        if (j >= 0) then !!!should be able to remove this?
                           if (j == 0) then
                              ...
                              gam(fc,j,s) = c_tri(fc,j-1,s) / bet
                              bet = b_tri(fc,j,s) - a_tri(fc,j,s) * gam(fc,j,s)
                     ...
                     do j = nlevdecomp,0,-1
                       if (j >= 0) then
                          conc_trcr(fc,j,s) = conc_trcr(fc,j,s) - gam(fc,j+1,s) * conc_trcr(fc,j+1,s)
                       end if
                     end do
                  end if
               end do
